...
namespace xbmcontrolevo
{
	...
	public class AlbumBrowser
	{
		...
		public AlbumBrowser(XbmControlEvo parent)
		{
			...
			tvcAlbumIcons 	= _parent.tvAlbums.AppendColumn ("", new CellRendererPixbuf (), "pixbuf", 0);
			tvcAlbumNames 	= _parent.tvAlbums.AppendColumn ("", new CellRendererText (), "text", 1);
			tvcAlbumIds 	= _parent.tvAlbums.AppendColumn ("", new CellRendererText (), "text", 2);
			...
			_parent.tvAlbums.ColumnsAutosize();
		}
				...
				if (aAlbums != null)
				{
					...
					for (int x = 0; x < aAlbums.Length; x++)
					{
						if (aAlbums[x] != "" && aAlbumIds[x] != "")
							tsAlbums.AppendValues (_parent.oImages.menu.cd, aAlbums[x], aAlbumIds[x]);
					}
					
					_parent.tvAlbums.Model = tsAlbums;
					_parent.tvAlbums.ShowAll();
				}
			}
		}
		...
		{
			if (_parent.tvAlbums.Selection.GetSelected(out selectedModel, out selectedIter))
